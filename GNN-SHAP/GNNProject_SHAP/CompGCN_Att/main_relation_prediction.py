...
def main(args):
    ...
    if args.model_path != "":
        compgcn_model.load_state_dict(th.load(args.model_path))
    ...
    for epoch in range(args.max_epochs):
        ...
        # data_new = Data(args.dataset, args.lbl_smooth, args.num_workers, args.batch_size)
        # data_iter_new = data.data_iter #train/validation/test data_loader
        # graph_new = data.g.to(device)
        ...
        for step, batch in enumerate(data_iter['train']):
            ...
            sub, rel, obj, label = triple[:, 0], triple[:, 1], triple[:, 2], label
            label = th.rand(label.shape[0], 22).to(device)
            pos_indices = th.arange(5, 120)
            neg_indices = th.arange(125, 900)
            # Code to mask the positive edges get the index of masked edges and make labels
            
            logits, node_features = compgcn_model(graph, sub, rel, obj) # shape is 1024, 474
            # label = th.rand(label.size()[0], 22).to(device)

            # compute loss
            tr_pos_loss = loss_fn(logits[pos_indices], label[pos_indices])  # classification loss
        ...
        graph.add_edges(sources, destinations)
        graph.edata['etype'][th.arange(len(graph.edata['etype']) - k, len(graph.edata['etype']))]  = relations
    ...
    with open(f"checkpoints/{args.run_name}/train_loss2.json", "w") as f:
        json.dump(losses, f)

    with open(f"checkpoints/{args.run_name}/val_mrr2.json", "w") as f:
        json.dump(mrrs, f)
